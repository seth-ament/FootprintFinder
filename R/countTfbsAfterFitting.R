...
foreach( tf=tflist ) %dopar% {
  ...
  load( prob_file )
  probs = bed[,-c(1:7)]
  prob = apply( probs , 1 , quantile , probs = 0.8 )
  x = which( prob > 0.2 )
  if( length(x) > 0 ) {
    fp = makeGRangesFromDataFrame( bed[ x , ] )
    counts = countOverlaps( tss.gr , fp , maxgap = 10000 )
  } 
  if( length(x) == 0 ) {
    counts = rep( 0 , length(tss.gr) )
  }
  return(counts)
}
names(tfbs.counts.10kb) = tflist
tfbs.counts.10kb = as.data.frame( tfbs.counts.10kb )
tfbs.counts.10kb = cbind( tss.loc , tfbs.counts.10kb )
save( tfbs.counts.10kb , file=paste("tfbs.counts.10kb.",Sys.Date(),".RData",sep=""))


# x = colSums( tfbs.counts.10kb[,-c(1:5)] > 0 )
